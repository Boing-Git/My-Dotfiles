...
ColumnLayout {
    ...
    function updateVariable(key, val) {
        var proc = Qt.createQmlObject('import Quickshell.Io; Process { command: ["omniformis", "qs", "set", "' + key + '", "' + val + '"]; onExited: destroy() }', rootPage);
        proc.running = true;
    }
    ...
    ColumnLayout {
        ...
        Text {
            text: "Shell interface, layout, and theme properties"
            font.family: Vars.fontFamily
            font.pixelSize: 12
            color: Theme.on_surface
            opacity: 0.7
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    var helpPart = "Manage " + key;
                    if (key.includes("radius") || key.includes("spacing") || key.includes("padding")) category = "Layout & Radii";
                    else if (key.includes("m3") || key.includes("animation")) category = "Animations & Colors";
                    else if (key.includes("overview")) category = "Overview Settings";
                    
                    // Don't clutter the UI with internal state variables
                    if (key === "notificationHistory" || key === "historyUpdated") continue;
                    
                    newVars.push({ key: key, type: type, help: helpPart, enums: "", val: valPart, category: category });
                }
